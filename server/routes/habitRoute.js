...
router.put('/updatehabit/:habitID', async (req, res) => {
    try {
        ...
        const updatedHabit = await Habit.findOneAndUpdate({_id:habitID}, { title: title, dates: [] }, { new: true });
        if(!updatedHabit) {
            res.status(404).send({msg: "Habit not found"});
        }
        else {
            // console.log(updatedHabit.title, updatedHabit.dates);
        
            res.status(200).json(updatedHabit);
        }
    } catch (error) {
        return res.status(500).send({mes: "Unable to process your request"})
    }
})
